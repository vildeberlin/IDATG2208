# Vi skal:
#   standardise the weak feature on the *training* set only
#   fit ŷ = w·x_std + b by vanilla gradient descent (MSE loss)
#   evaluate on the held‑out test set → MSE, RMSE, R²
...
# ---
# Svak predictor (smallest absolute ρ with the target)
features = df.columns.drop("critical_temp")
corr = df[features].corrwith(df["critical_temp"])
weak_feat = corr.abs().idxmin()
weak_corr = corr[weak_feat]

print("\n=== Weak predictor (used for all folds) ===")
print(f"Feature : {weak_feat}")
...
# ----
# 5‑fold random split (80 % trene – 20 % teste)
...
for fold in range(5):
    ...
    # ----- standardise weak feature on training data only ----------------
    x_train_raw = train[weak_feat].values.astype(np.float64)
    ...
    x_test_raw = test[weak_feat].values.astype(np.float64)
...
# ---
# Print resultatene
print("\n=== 5‑fold evaluation (weak predictor only) ===")
