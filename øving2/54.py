#!/usr/bin/env python3
# --------------------------------------------------------------
#  Decision‑Tree depth sweep – visualise boundary complexity
# --------------------------------------------------------------
...
# ------------------------------------------------------------------
# 6) Fit preprocessing on the training part only
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# 7) Subsample 10 k (keep class proportions)
# ------------------------------------------------------------------
SUBSIZE = 10_000
...
# ------------------------------------------------------------------
# 8) Reduce to 2 D with PCA (same projection for all trees)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# 9) Plot decision boundaries for increasing max_depth
# ------------------------------------------------------------------
depths = [2, 5, 10, 20]          # four settings, from very shallow to deep
fig, axes = plt.subplots(1, len(depths), figsize=(4*len(depths), 4),
                         sharex=True, sharey=True)

def plot_tree(ax, depth):
    tree = DecisionTreeClassifier(max_depth=depth,
                                  criterion='entropy',   # the best criterion from Exercise 2
                                  min_samples_leaf=20,
                                  random_state=42)
    ...
    for cls, col, mk in zip([0, 1], colors, markers):
        ...
        ax.scatter(X_val_2d[idx,0], X_val_2d[idx,1],
                   ...
                   s=30, label=f"{'<=50K' if cls==0 else '>50K'}",
                   alpha=0.8)
...
plt.suptitle("Decision‑Tree boundary evolution with increasing max_depth\n(Validation points in true colours)")
